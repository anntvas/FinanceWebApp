...
from rest_framework.response import Response
from .ml_service import predict_future_expenses
...
class PredictView(APIView):
    ...
    def get(self, request):
        transactions = Transaction.objects.filter(
            budget_category__group__budget__owner=request.user
        ).values('date', 'amount')

        data_list = list(transactions)
        ...
        if df.empty:
            return Response({'forecast': [], 'debug': 'DataFrame empty', 'raw': data_list})

        try:
            forecast = predict_future_expenses(df)
        except Exception as e:
            return Response({
                'forecast': [],
                'error': str(e),
                'debug': 'forecasting failed',
                'raw': data_list
            })
        return Response({'forecast': forecast, 'raw': data_list})
